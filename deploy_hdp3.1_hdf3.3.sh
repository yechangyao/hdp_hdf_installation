...
export minfi_toolkit="http://public-repo-1.hortonworks.com/HDF/3.3.0.0/minifi-toolkit-0.6.0.3.3.0.0-165-bin.tar.gz"
...
export ambari_services="HDFS MAPREDUCE2 YARN ZOOKEEPER DRUID SUPERSET STREAMLINE NIFI NIFI_REGISTRY KAFKA STORM REGISTRY HBASE PHOENIX" ## ZEPPELIN SPARK2
...
echo "installing MiNiFi java"
wget ${minifi_java}
wget ${minfi_toolkit}
cp minifi-0.*-bin.tar.gz /usr/hdf
cp minifi-toolkit-0.*-bin.tar.gz /usr/hdf
cd /usr/hdf
tar zxvf minifi-0.*-bin.tar.gz
tar xzvf minifi-toolkit-0.*-bin.tar.gz
